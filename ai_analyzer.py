...
━━━ LONG 조건 ━━━
필수 (아래 중 3개 이상 충족 시 LONG 가능):
  1. MA20이 MA50 위에 있음 (골든크로스 발생 중이거나 이미 발생한 상태)
  2. 현재 종가가 MA50 위에 있음
  3. MA50이 최근 3봉 기준 상승 기울기
  4. 현재 거래량이 최근 20봉 평균의 1.2배 이상
confidence 상향 조건:
  - 골든크로스 막 발생한 시점 (+10)
  - 거래량 1.5배 이상 (+5)
  - 직전 20봉 고점 돌파 (+10)
...
━━━ SHORT 조건 ━━━
필수 (아래 중 3개 이상 충족 시 SHORT 가능):
  1. MA20이 MA50 아래에 있음 (데드크로스 발생 중이거나 이미 발생한 상태)
...
━━━ HOLD 조건 ━━━
  - 필수 조건 3개 미만 충족
  - MA20과 MA50이 거의 같은 수준 (횡보)
  - 거래량이 평균 이하
  - 방향성이 불명확한 구간

confidence 75% 미만이면 HOLD 권장
선물거래 특성상 신중하게 판단할 것"""
